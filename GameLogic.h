...
#define ST_MISSILE_FIRST            2									//王先排序
#define ST_NORMAL                   3									//普通
#define ST_SPECIAL                  4									//特殊
...
#define CT_ALLTYPE_CARD             21                                  //所有支持的牌型
...
{
    BYTE							cbCardType[10];             //类型
    BYTE							cbCardCount[10];			//数目
};
...
//    BYTE							cbEachCardType[CT_ALLTYPE_CARD];   //因在Upside等函数判断中，将火箭炸和同牌炸放在一起，因此需要增加变量区分炸弹类型 add by cx
};
...
public:
    ...
    //分析炸弹 在原来的基础上增加大小排序
    //cbSortType为ST_ORDER表示由大到小
    //cbSortType为ST_KINGFIRST表示王在前，后面的扑克由大到小
