...
//扑克数据
const BYTE	CGameLogicold::m_cbCardData[FULL_COUNT]=
{
...
const BYTE	CGameLogicold::m_cbGoodcardData[GOOD_CARD_COUNT]=
{
0x01,0x02,
0x11,0x12,
0x21,0x22,
...
//去掉3~9 其余double
const BYTE	CGameLogicold::m_cbGoodcardData2[FULL_COUNT]=
{
        0x01,0x01,0x02,0x02,0x0A,0x0A,0x0B,0x0B,0x0C,0x0C,0x0D,0x0D,	//方块 A - K
        0x11,0x11,0x12,0x12,0x1A,0x1A,0x1B,0x1B,0x1C,0x1C,0x1D,0x1D,	//梅花 A - K
        0x21,0x21,0x22,0x22,0x2A,0x2A,0x2B,0x2B,0x2C,0x2C,0x2D,0x2D,	//红桃 A - K
        0x31,0x31,0x32,0x32,0x3A,0x3A,0x3B,0x3B,0x3C,0x3C,0x3D,0x3D,    //黑桃 A - K
...
//获取类型
BYTE CGameLogicold::GetCardType(const BYTE cbCardData[], BYTE cbCardCount)
{
	//简单牌型
	switch (cbCardCount)
	{
    ...
    /************************************************************************/
    /* 飞机组合牌型判断逻                                                   */
    /* 连续的四个 如AAAABBBB 可算作3A 3B 带AB 飞机                          */
    /* 连续的5个 也类似                                                     */
    /************************************************************************/
    ...
    //两副牌新增牌型
    if (AnalyseResult.cbFiveCount > 0)
    {
        return CT_BOMB_CARD_5;
    }

    if (AnalyseResult.cbSixCount > 0)
    {
        return CT_BOMB_CARD_6;
    }

    if (AnalyseResult.cbSenvenCount > 0)
    {
        return CT_BOMB_CARD_7;
    }

    if (AnalyseResult.cbEightCount > 0)
    {
...
//排列扑克
void CGameLogicold::SortCardList(BYTE cbCardData[], BYTE cbCardCount, BYTE cbSortType)
{
	//数目过虑
...
//得到好牌
void CGameLogicold::GetGoodCardData(BYTE cbGoodCardData[NORMAL_COUNT])
{
	//混乱准备
...
//删除好牌
bool CGameLogicold::RemoveGoodCardData(BYTE cbGoodcardData[], BYTE cbGoodCardCount, BYTE cbCardData[], BYTE cbCardCount) 
{
	//检验数据
...
//混乱扑克
void CGameLogicold::RandCardList(BYTE cbCardBuffer[], BYTE cbBufferCount)
{
	//混乱准备
...
void CGameLogicold::RandCardList(BYTE cbCardBuffer[], BYTE cbBufferCount, BYTE bMode)
{
    BYTE cbCardData[CountArray(m_cbCardData)] = {0};
    BYTE cbRandCount = 0;
    BYTE cbPosition = 0;
...
//删除扑克
bool CGameLogicold::RemoveCard(const BYTE cbRemoveCard[], BYTE cbRemoveCount, BYTE cbCardData[], BYTE cbCardCount)
{
	//检验数据
...
//有效判断
bool CGameLogicold::IsValidCard(BYTE cbCardData)
{
	//获取属性
...
//逻辑数值
BYTE CGameLogicold::GetCardLogicValue(BYTE cbCardData)
{
	//扑克属性
	BYTE cbCardColor=GetCardColor(cbCardData);
	BYTE cbCardValue=GetCardValue(cbCardData);
...
//对比扑克
bool CGameLogicold::CompareCard(const BYTE cbFirstCard[], const BYTE cbNextCard[], BYTE cbFirstCount, BYTE cbNextCount)
{
	//获取类型
...
//分析扑克
bool CGameLogicold::AnalysebCardData(const BYTE cbCardData[], BYTE cbCardCount, tagAnalyseResult & AnalyseResult)
{
	//设置结果
...
//随机扑克
BYTE CGameLogicold::GetRandomCard(void)
{
...
//出牌搜索
bool CGameLogicold::SearchOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, WORD wOutCardUser, WORD wMeChairID, tagOutCardResult & OutCardResult)
{
	//玩家判断
...
//分析炸弹
void CGameLogicold::GetAllBomCard(BYTE const cbHandCardData[], BYTE const cbHandCardCount, BYTE cbBomCardData[], BYTE &cbBomCardCount)
{
...
//分析顺子
void CGameLogicold::GetAllLineCard(BYTE const cbHandCardData[], BYTE const cbHandCardCount, BYTE cbLineCardData[], BYTE &cbLineCardCount)
{
...
//分析三条
void CGameLogicold::GetAllThreeCard(BYTE const cbHandCardData[], BYTE const cbHandCardCount, BYTE cbThreeCardData[], BYTE &cbThreeCardCount)
{
	BYTE cbTmpCardData[MAX_COUNT] ;
	CopyMemory(cbTmpCardData, cbHandCardData, cbHandCardCount) ;
...
//分析对子
void CGameLogicold::GetAllDoubleCard(BYTE const cbHandCardData[], BYTE const cbHandCardCount, BYTE cbDoubleCardData[], BYTE &cbDoubleCardCount)
{
	BYTE cbTmpCardData[MAX_COUNT] ;
	CopyMemory(cbTmpCardData, cbHandCardData, cbHandCardCount) ;
...
//分析单牌
void CGameLogicold::GetAllSingleCard(BYTE const cbHandCardData[], BYTE const cbHandCardCount, BYTE cbSingleCardData[], BYTE &cbSingleCardCount)
{
	cbSingleCardCount =0 ;
...
//分析出牌
void CGameLogicold::AnalyseOutCardType(BYTE const cbHandCardData[], BYTE const cbHandCardCount, tagOutCardTypeResult CardTypeResult[12+1])
{
...
//分析牌型
void CGameLogicold::AnalyseOutCardType(BYTE const cbHandCardData[], BYTE const cbHandCardCount, BYTE const cbTurnCardData[], BYTE const cbTurnCardCount,tagOutCardTypeResult CardTypeResult[12+1])
{
	ZeroMemory(CardTypeResult, sizeof(tagOutCardTypeResult)*12) ;
...
*********************************************************************/
//组合算法
void CGameLogicold::Combination(BYTE cbCombineCardData[], BYTE cbResComLen,  BYTE cbResultCardData[100][5], BYTE &cbResCardLen,BYTE cbSrcCardData[] , BYTE cbCombineLen1, BYTE cbSrcLen, const BYTE cbCombineLen2)
{
...
//排列算法
void CGameLogicold::Permutation(BYTE *list, int m, int n, BYTE result[][4], BYTE &len)
{ 
	int j,temp; 
	if(m == n){ 
		for(j = 0; j < n; j++) 
...
//单牌个数
BYTE CGameLogicold::AnalyseSinleCardCount(BYTE const cbHandCardData[], BYTE const cbHandCardCount, BYTE const cbWantOutCardData[], BYTE const cbWantOutCardCount, BYTE cbSingleCardData[])
{
	//参数判断
	...
	//函数指针
	typedef void (CGameLogicold::*pGetAllCardFun)(BYTE const [], BYTE const , BYTE[], BYTE &); 

	//指针数组
	pGetAllCardFun GetAllCardFunArray[4] ;
	GetAllCardFunArray[0] = &CGameLogicold::GetAllBomCard ;		//炸弹函数
	GetAllCardFunArray[1] = &CGameLogicold::GetAllLineCard ;	//顺子函数
	GetAllCardFunArray[2] = &CGameLogicold::GetAllThreeCard ;	//三条函数
	GetAllCardFunArray[3] = &CGameLogicold::GetAllDoubleCard ;	//对子函数
...
//设置扑克
void CGameLogicold::SetUserCard(WORD wChairID, BYTE cbCardData[], BYTE cbCardCount)
{
...
//设置底牌
void CGameLogicold::SetBackCard(WORD wChairID, BYTE cbBackCardData[], BYTE cbCardCount)
{
...
//设置庄家
void CGameLogicold::SetBanker(WORD wBanker) 
{
	m_wBankerUser = wBanker ;
	m_lBankerOutCardCount = 0 ;
}
//叫牌扑克
void CGameLogicold::SetLandScoreCardData(BYTE cbCardData[], BYTE cbCardCount) 
{
...
//删除扑克
void CGameLogicold::RemoveUserCardData(WORD wChairID, BYTE cbRemoveCardData[], BYTE cbRemoveCardCount) 
{
...
//地主出牌（先出牌）
void CGameLogicold::BankerOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, tagOutCardResult & OutCardResult) 
{
	//零下标没用CT_JET
	tagOutCardTypeResult CardTypeResult[12+1] ;
	ZeroMemory(CardTypeResult, sizeof(CardTypeResult)) ;
...
//地主出牌（后出牌）
void CGameLogicold::BankerOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, WORD wOutCardUser, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, tagOutCardResult & OutCardResult) 
{
	//初始变量
	ZeroMemory(&OutCardResult, sizeof(OutCardResult)) ;
	...
	{
		//最小单牌
        BYTE cbSingleCardData[20] = {0};
        BYTE cbTmpCount = AnalyseSinleCardCount(cbHandCardData, cbHandCardCount,CanOutCard.cbCardData[i], CanOutCard.cbEachHandCardCount[i], cbSingleCardData) ; 
		BYTE cbMaxValue=0 ; 
		BYTE Index = 0 ;
...
//地主上家（先出牌）
void CGameLogicold::UpsideOfBankerOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, WORD wMeChairID, tagOutCardResult & OutCardResult)
{
	//初始变量
	ZeroMemory(&OutCardResult, sizeof(OutCardResult)) ;
...
//地主上家（后出牌）
void CGameLogicold::UpsideOfBankerOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, WORD wOutCardUser, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, tagOutCardResult & OutCardResult) 
{
	//零下标没用
	tagOutCardTypeResult CardTypeResult[12+1] ;
	ZeroMemory(CardTypeResult, sizeof(CardTypeResult)) ;
...
//地主下家（先出牌）
void CGameLogicold::UndersideOfBankerOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, WORD wMeChairID,tagOutCardResult & OutCardResult) 
{
	//零下标没用
	tagOutCardTypeResult CardTypeResult[12+1] ;
	ZeroMemory(CardTypeResult, sizeof(CardTypeResult)) ;
...
//地主下家（后出牌）
void CGameLogicold::UndersideOfBankerOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, WORD wOutCardUser, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, tagOutCardResult & OutCardResult)
{
	//初始变量
	ZeroMemory(&OutCardResult, sizeof(OutCardResult)) ;
...
//叫分判断
BYTE CGameLogicold::LandScore(WORD wMeChairID, BYTE cbCurrentLandScore, BYTE cbCallScorePhase)
{
	//大牌数目
	...
	//叫一分
	if(cbLargeCardCount>=3 && cbSingleCardCount<cbMinSingleCardCount-3) return 255 ;
...
//出牌测试
bool CGameLogicold::_TestOutAllCard(WORD wTestUser, BYTE cbWantOutCardData[], BYTE cbWantOutCardCount, BYTE	cbAllCardData[GAME_PLAYER][MAX_COUNT], BYTE cbUserCardCount[GAME_PLAYER], bool bFirstOutCard)
{
	//此递归暂不可用
	//return false ;
...
//四带牌型
bool CGameLogicold::AnalyseFourCardType( BYTE const cbHandCardData[MAX_COUNT], BYTE cbHandCardCount, BYTE cbEnemyCardData[MAX_COUNT], BYTE cbEnemyCardCount, tagOutCardResult &CardResult )
{
	//初始变量
...
//出牌测试
bool CGameLogicold::TestOutAllCard( WORD wTestUser, BYTE cbWantOutCardData[], BYTE cbWantOutCardCount, bool bFirstOutCard ) {
...
//最大判断
bool CGameLogicold::IsLargestCard( WORD wTestUser, BYTE const cbWantOutCardData[], BYTE const cbWantOutCardCount ) {
...
//是否能出
bool CGameLogicold::VerifyOutCard( WORD wTestUser, BYTE const cbWantOutCardData[], BYTE const cbWantOutCardCount, BYTE const cbCurHandCardData[ MAX_COUNT ], BYTE const cbCurHandCardCount, bool bFirstOutCard ) {
...
//出牌搜索
bool CGameLogicold::SearchOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, tagOutCardResult & OutCardResult)
{
    //设置结果
    ZeroMemory(&OutCardResult,sizeof(OutCardResult));
...
/************************************************************************/
/* 根据分析结果 判断是否飞机牌型                                        */
/* 引入两幅牌后 会多出来很多组合牌型的飞机                              */
/************************************************************************/
//bool CGameLogicold::IsJetCardType(const BYTE cbCardData[], BYTE cbCardCount)
//{
//    if (NULL == cbCardData)
//    {
//        return false;
...
//获取飞机牌型里最大的3张牌数值
//
BYTE CGameLogicold::GetJetMaxLogicValue(const BYTE cbCardData[], BYTE cbCardCount)
{
    if (NULL == cbCardData)
    {
        return 0;
...
bool CGameLogicold::IsJetCardType(const BYTE cbCardData[], BYTE cbCardCount)
{
    ...
    BYTE bFirstThreeCardData = 0;
    BYTE bLastThreeCardData = 0;
    BYTE bThreeCardData[15] = {0}; //记录所有三张牌的数值从零开始存 牌的值
    ...
    for (i = 3; i < 15; i++)
    {
        ...
        if (cbCardCountSum[i] >= 3)
        {
            bThreeCardData[bThreeCardCount] = i;
            bThreeCardCount++;
            bFirstThreeCardData = i;
            break;
        }
    ...
    while (bTempCardCount >= 3)
    {
        ...
        bCardIndex++;
        if (cbCardCountSum[bCardIndex] < 3 || bCardIndex > 14)
        {
            //bThreeCardCount = 0;
            bLastThreeCardData = 0;
        }
        else
        {
            bThreeCardData[bThreeCardCount] = bCardIndex;
            bThreeCardCount++;
            bLastThreeCardData = bCardIndex;
        }
        bTempCardCount -= cbCardCountSum[bCardIndex];
    ...
    if (bWingCardCount == bThreeCardCount) /*带单*/
    {
        ...
        if (SequentialThreeDataCount(bThreeCardData, 15) < bThreeCardCount)
        {
            return false;
        }
        else
    ...
    else if (bWingCardCount == (2*bThreeCardCount)) /*带队*/
    {
        ...
        //判断带队情况 极端情况会存在分拆4个 或6个成对子的情况
        //检测bWingCardData是不是全是对子
        if(IsAllCouple(bWingCardData, bWingCardCount))
        {            
            if (SequentialThreeDataCount(bThreeCardData, 15) < bThreeCardCount)
            {
                return false;
    ...
    else
    {
        ...
        if (bThreeCardCount > 2)
        {
            //可以拆第一个3张 或者最后一个3张作为翅膀
            /*
             * 到这个分支， 多于三个的分支已经处理3张牌到飞机里面，剩余的牌放到翅膀里。
             * 这里需要处理下，把三个拆回去组成新的翅膀
             *
             */
            if ((bWingCardCount + 3) == (bThreeCardCount - 1)
                || ((bWingCardCount + 3) == (bThreeCardCount - 1)))
            {
                //拆后 成带单牌飞机
                return true;
            }
            
            if ((bWingCardCount + 3) == 2* (bThreeCardCount - 1))
            {
                //拆最小的三个
                memset(bWingCardData + bWingCardCount, bFirstThreeCardData, sizeof(BYTE)*3);
                
                if(IsAllCouple(bWingCardData, (bWingCardCount + 3)))
                {
                    if (SequentialThreeDataCount(bThreeCardData, 15) < (bThreeCardCount - 1))
                    {
                        IsJet = false;
                    }
                    else
            ...
            {
                //拆最大的三个
                memset(bWingCardData + bWingCardCount, bLastThreeCardData, sizeof(BYTE)*3);
                if(IsAllCouple(bWingCardData, (bWingCardCount + 3)))
                {
                    if (SequentialThreeDataCount(bThreeCardData, 15) < (bThreeCardCount - 1))
                    {
...
//判断是否全是对子
bool CGameLogicold::IsAllCouple(const BYTE cbCardData[], BYTE cbCardCount)
{
    if (NULL == cbCardData)
    {
        return false;
...
bool CGameLogicold::SearchFiveBumb(const BYTE cbHandCardData[], BYTE cbHandCardCount, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, tagOutCardResult & OutCardResult,tagAnalyseResult AnalyseHandCard)
{
    //变量定义
    BYTE cbLogicValue = GetCardLogicValue(cbTurnCardData[0]);


    //两种情况需要寻找五张牌的炸弹，1 上家出了王炸  2 上家出了5张牌的炸弹
    if(cbTurnCardCount == 2)
    {
...
bool CGameLogicold::SearchSixBumb(const BYTE cbHandCardData[], BYTE cbHandCardCount, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, tagOutCardResult & OutCardResult,tagAnalyseResult AnalyseHandCard)
{
    //变量定义
    BYTE cbLogicValue = GetCardLogicValue(cbTurnCardData[0]);
...
bool CGameLogicold::SearchSevenBumb(const BYTE cbHandCardData[], BYTE cbHandCardCount, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, tagOutCardResult & OutCardResult,tagAnalyseResult AnalyseHandCard)
{
    BYTE cbLogicValue = GetCardLogicValue(cbTurnCardData[0]);
...
bool CGameLogicold::SearchEightBumb(const BYTE cbHandCardData[], BYTE cbHandCardCount, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, tagOutCardResult & OutCardResult,tagAnalyseResult AnalyseHandCard)
{
    BYTE cbLogicValue = GetCardLogicValue(cbTurnCardData[0]);
...
int CGameLogicold::CountJokers(const BYTE cbCardData[], BYTE cbCardCount)
{
    int nCountNumbers = 0;
...
int CGameLogicold::CountBigJoker(const BYTE cbCardData[], BYTE cbCardCount)
{
    int nCountNumbers = 0;
    for(int i = 0; i < cbCardCount;i++)
    {
...
BYTE CGameLogicold::SequentialThreeDataCount(BYTE bCardData[], BYTE bCardCount)
{
    BYTE bCount = 1;
    BYTE i = 0;
